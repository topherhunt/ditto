...
describe("practice helpers", () => {
  ...
  it("locks correct and accent-fixed slots, keeping typed punctuation, and keeps wrong ones editable", () => {
    const r = grade({ mode: "slots", slots: ["vorrei", "un", "caffe,", "per", "favor"] }, { language: "it", text: "Vorrei un caffè, per favore." });
  ...
  it("keeps a slot with a wrong end mark open", () => {
    const r = grade({ mode: "slots", slots: ["vorrei", "un", "caffè?"] }, { language: "it", text: "Vorrei un caffè." });
